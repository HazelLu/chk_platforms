      ...
      if(app._getLocalStorageJson('com_count')==0){
        ...
        if(app._getLocalStorageJson('com_count')==0){
            ...
            if (app._getLocalStorageJson('com_count') > 1) {
                ...
                storeObject.someValue=JSON.parse(app._getLocalStorage('comInfo')).info[0].id;
                _mobile.navigate('reservation_v2.html', { changeHash: true});
            }
          /*
            if (app._getLocalStorageJson('com_count') > 1) {
...
//window.plugins.socialsharing.share('370/月，70M', null, null, 'http://www.kingnet.net.tw/userfiles/0519-%E8%B3%87%E8%B2%BB%E8%88%87%E6%9C%8D%E5%8B%99-01%20(1).png')
    ...
    */
    page.on('click', '#user_list a', function () {
        if(nextPage=='reservation_v2.html'){
            storeObject.someValue=$(this).find('input[type^=hidden]').val();
            _mobile.navigate(nextPage,{changeHash: true});
        }else{
...
document.removeEventListener("backbutton",_onBackPageAnn, false);
 ...
 app._ga_trackView('reservationDate','');
  ...
  var res_jq=$(this);
  var res=this;
  var com_id=storeObject.someValue;
  var pf_id=storeObject.anotherValue;
  var tempStart;
  var tempDue;
  var tempPeople;
  var tempHour=1;
  ...
  res_jq.on('click','#startBtn,#dueBtn',function(){  
    ...
    storeObject.someValue=com_id;
    storeObject.anotherValue=pf_id;
   _mobile.navigate('reservationDetail.html', { changeHash: true,reverse:true });
                         ...
                         navigator.notification.confirm(
                            ...
                            function (idx) {
                                ...
                                if(idx==1){
                                  ...
                                  _mobile.navigate('reservationHistory_v2.html', { changeHash: true,reverse:true }); 
                                }else{
                                  _mobile.navigate('reservationHistory_v2.html', { changeHash: true,reverse:true });         
                                }
                            },
   ...
   var com_id=storeObject.someValue;
   var pf_id=storeObject.anotherValue;
   var date=storeObject.threeValue;
   var config;
      ...
      datePicker.show(options, function(date){
         ...
         if(dayopen=='none'){
          app._dialog(d+'('+app._getWeekDay(d)+')該公設未開放');
            ...
            storeObject.someValue=com_id;
            storeObject.anotherValue=pf_id;
            storeObject.threeValue=d;
          _mobile.navigate('reservationDate_v2.html', { changeHash: true });   
         }  
       }else{
          app._dialog('請選擇正確日期')
       }
 ...
 $.each(JSON.parse(app._getLocalStorage('facilities')), function (i, v) {
      if(v.pf_id==pf_id){
        ...
        res.querySelector('#max_people').innerText=v.res_attendLimit == 0 ? '--':v.res_attendLimit;
        res.querySelector('#max_res').innerText=v.maximum_people == 0 ? '--':v.maximum_people;
  ...
  res_jq.on('click', '#header a:eq(0)', function () {
    if (app._getLocalStorageJson('com_count') > 1) {
        ...
        storeObject.someValue=com_id;
       _mobile.navigate('reservationHistory_v2.html', { changeHash: true });
  }).on('click','#resDomList li a,#heartDomList li a',function(){
    //app._loadingShow();
    storeObject.someValue=com_id;
    storeObject.anotherValue=this.querySelector('input').value;
    //res.querySelector('#resDomList').innerHTML='';
    _mobile.navigate('reservationDetail.html', { changeHash: true});
  }).on('click','.filterTips .clear',function(){
  ...
  temp1='';
    ...
    var resHistory=$(this);
      var com_id=storeObject.someValue;
         var nav=storeObject.anotherValue;
      temp1=com_id;
    var res=this;
    var _showTab = 0;
    if(nav=='record'){
                ...
                if(idx==1){
                  app._setCancelReservation(resId,function(msg){
                    if(msg=='success'){
                        app._dialog('取消預約成功');
                        storeObject.someValue=com_id;
                        storeObject.anotherValue='record';
...
app._ga_trackView('reservationHistory','');
  ...
  _map.setClickable(true); 
    ...
    easylife.on('click','#header a:eq(0)',function(){
        ...
        var txt=this.querySelector('h4').innerText;
        window.ga.trackEvent('easylife', txt, txt, 1); 
        if(val[0].value==0){
    ...
    app._footerShow('easylife.html',this.id);
      ...
      app._getEasyLifeList(function(datas){
        if(datas !='error'){
             ...
             easylife.find('ul.list').html(easylife.find('ul.list').html()+domStr);
             if(checkStore==true){
